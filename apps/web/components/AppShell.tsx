        ...
        <main className="main-column">
          ...
          <div className="canvas-scroll">
            ...
            {composition && loadState === "success" && <>
              ...
              {messages.length > 0 && <section className="conversation-stream" aria-live="polite">{messages.map((message) => <article className={`chat-message chat-${message.role}`} key={message.id}><span>{message.role === "assistant" ? "JARVIS" : "YOU"}</span><p>{message.content}</p>{message.response && <><div className="chat-evidence"><code>{message.response.findings.length} findings</code><code>{message.response.evidence_refs.length} evidence refs</code><code>synthetic={String(message.response.synthetic)}</code></div><details className="chat-finding-details"><summary>查看 Findings 与证据</summary>{message.response.findings.map((finding) => <div className="chat-finding" key={finding.finding_id}><div><strong>{finding.finding_type}</strong><span>{finding.causal_status} · {Math.round(finding.confidence * 100)}%</span></div><p>{finding.claim}</p><dl><div><dt>Data period</dt><dd>{finding.data_period.start.slice(0, 10)} → {finding.data_period.end.slice(0, 10)}</dd></div><div><dt>Source</dt><dd>{finding.source.join(", ")}</dd></div><div><dt>Updated</dt><dd>{new Date(finding.updated_at).toLocaleString()}</dd></div><div><dt>Synthetic</dt><dd>{String(finding.synthetic)}</dd></div></dl>{finding.evidence_refs.map((ref) => <code key={`${ref.kind}:${ref.reference_id}`}>{ref.kind} · {ref.reference_id}</code>)}{finding.limitations.length > 0 && <small>限制：{finding.limitations.join("；")}</small>}</div>)}</details></>}</article>)}{chatting && <article className="chat-message chat-assistant"><span>JARVIS</span><p>正在调用 Store Operations Agent 与确定性工具…</p></article>}</section>}
